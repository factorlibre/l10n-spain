...
class TestL10nEsAeatMod303VatProrate(TestL10nEsAeatMod303Base):
    ...
    def _get_taxes(self, descs):
        ...
        for desc in descs.split(","):
            ...
            if tax_template:
                tax = self.env["l10n.es.aeat.report"].get_taxes_from_templates(
                    tax_template
                )
                taxes |= tax
        return taxes
    ...
    # SE DEJA COMENTADO POR AHORA, NO ESTÁ FUNCIONANDO CORRECTAMENTE EN EL CASO
    # DE PRORATA ESPECIAL, PERO NO SE LE VA A DAR USO POR AHORA
    ...
    #     # create invoices + model 303 for 1T
    #     self._invoice_sale_create("2017-01-01")
    #     self._invoice_purchase_create("2017-01-01", p_inv_extra_data)
    #     self.model303.button_calculate()
    #     self.assertEqual(self.model303.total_devengado, 210)
    ...
    #     # create invoices + model 303 for 4T
    #     self._invoice_sale_create("2017-11-01")
    #     self._invoice_purchase_create("2017-11-01", p_inv_extra_data)
    #     with self.assertRaises(exceptions.ValidationError):
    #         self.model303_4t.vat_prorate_percent = 101
